...
def getLaneCurve(img,display=2):
    ...
    points = [137,0,100,240]
    widthTop,heightTop,widthBottom,heightBottom = 137,0,100,240
    wT=480
    hT=240
    ...
    if display != 0:
        ...
        if curve < -20:
            cv2.putText(imgResult,str(curve)+" Turn Left",(w//2-80,85),cv2.FONT_HERSHEY_COMPLEX,1,(255,0,255),3)

        if curve > 20:
            cv2.putText(imgResult,str(curve)+" Turn Right",(w//2-80,85),cv2.FONT_HERSHEY_COMPLEX,1,(255,0,255),3)
        if (curve >0 and curve<20) or (curve >-20 and curve <0):
            cv2.putText(imgResult,str(curve)+" Forward",(w//2-80,85),cv2.FONT_HERSHEY_COMPLEX,1,(255,0,255),3)

        if goal_reached:
            print("Goal Reached")
            cv2.putText(imgResult,"Goal Reached",(w//2-80,130),cv2.FONT_HERSHEY_COMPLEX,1,(0,0,255),3)
